...
const char *ssid = "TIMEOSPACE";    // Ganti dengan SSID WiFi Anda
const char *password = "1234Saja";  // Ganti dengan password WiFi Anda
...
void setup() {
  ...
  // Connect to WiFi using the library method
  display.connectToWiFi(ssid, password, 15);  // Timeout 15 detik

  // Create main menu with 3 items (tanpa "WiFi")
  mainMenu = display.createMenu(3, "Display", "System", "About");
...
void loop() {
  ...
  display.onSelect(
    ...
    [](MenuCursor *cursor) {
      // Use the renderInfoScreen function
      String tempStr = "Temp: " + String(temperatureValue) + "C";
      String uptimeStr = "Uptime: " + String(millis() / 1000) + "s";
      String wifiStr = "WiFi: " + display.getWiFiStatus();  // Menggunakan method library
  ...
  display.onSelect(
    ...
    [](MenuCursor *cursor) {
      // Use the renderInfoScreen function
      String ipText = "IP: " + display.getIPAddress();  // Menggunakan method library
