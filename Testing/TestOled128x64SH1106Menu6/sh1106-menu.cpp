...
// Implementations without listenSerialInput
...
bool SH1106Menu::showConfirmation(const char *title, const char *message) {
  ...
  while (deciding) {
    ...
    int btnHeight = 15;
    int btnGap = 5;
    int btnX = (displayWidth - btnWidth) / 2;
    int yesY = 35;
    int noY = yesY + btnHeight + btnGap;

    if (selected) {  // Ya selected
      setColor(WHITE);
      fillRect(btnX, yesY, btnWidth, btnHeight);
      setColor(BLACK);
      drawString(btnX + btnWidth / 2, yesY + 3, "Ya");

      setColor(WHITE);
      drawRect(btnX, noY, btnWidth, btnHeight);
      drawString(btnX + btnWidth / 2, noY + 3, "Tidak");
    } else {  // Tidak selected
      setColor(WHITE);
      drawRect(btnX, yesY, btnWidth, btnHeight);
      drawString(btnX + btnWidth / 2, yesY + 3, "Ya");

      setColor(WHITE);
      fillRect(btnX, noY, btnWidth, btnHeight);
      setColor(BLACK);
      drawString(btnX + btnWidth / 2, noY + 3, "Tidak");
    }
...
void SH1106Menu::showScrollableText(const char *title, const char *text) {
  ...
  while (!exiting) {
    ...
    for (int i = 0; i <= currentLine + maxLinesPerPage && i < 100; i++) {
      String line = "";
    ...
    if (currentLine + maxLinesPerPage < 100) {
    ...
    if (cursor_->down && currentLine + maxLinesPerPage < 100) {
      currentLine++;
      cursor_->down = false;
    }
...
int SH1106Menu::showSlider(const char *title, int minValue, int maxValue, int currentValue) {
  ...
  bool exiting = false;
  bool valueChanging = true;  // Flag to toggle between changing value and selecting
...
void SH1106Menu::showTextInput(const char *title, char *buffer, int maxLength) {
  ...
  bool exiting = false;
  bool modeSelectChar = true;  // true: select character, false: confirm character
  ...
  while (!exiting) {
    ...
    setTextAlignment(TEXT_ALIGN_CENTER);
    drawString(displayWidth / 2, 20, currentText);

    // Show cursor at correct position
    if (millis() % 1000 < 500) {
      int cursorX = (displayWidth / 2) - (getStringWidth(currentText.c_str()) / 2);
      for (int i = 0; i < cursorPos; i++) {
        cursorX += getStringWidth(String(buffer[i]).c_str());
      }
      drawLine(cursorX, 20, cursorX, 28);
    }
    ...
    if (modeSelectChar) {
      ...
      drawString(displayWidth / 2, 32, "^");
      drawString(displayWidth / 2, 40, String(currChar));
      drawString(displayWidth / 2, 48, "v");

      drawString(displayWidth / 2, 56, "Select untuk masukkan");
    } else {
      setTextAlignment(TEXT_ALIGN_CENTER);
      drawString(displayWidth / 2, 40, "Select: tambah, Back: selesai");
      drawString(displayWidth / 2, 50, "^ v untuk karakter lain");
    }
